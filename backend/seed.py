...
def seed():
    ...
    pr1 = ProcurementService.create_pr(
        ...
        title="Sắm 3 Laptop Dell Vostro Workstation",
        items=[{"itemName": "Laptop Dell Vostro Workstation", "quantity": 3, "estimatedUnitPrice": 25_000_000}]
    )
    print(f"✅ Created PR 1: {pr1['id']} (Estimated: {pr1['estimatedValue']:,.0f} VND)")

    # Approve PR1
    pr1_approved = ProcurementService.approve_pr(
        pr_id=pr1["id"],
        approver_role="MANAGER",
        approver_name="Trần Văn B (Manager)",
        comments="Đã duyệt phục vụ dự án mới"
    )
    print(f"✅ Approved PR 1 Step 1: Status = {pr1_approved['status']}")
